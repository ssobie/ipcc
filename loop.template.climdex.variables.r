...
var.name <- input.varname[[clim.name]]
print(var.name)
type <- 'annual'
...
for (gcm in gcm.list) {
   ...
   for (scenario in scenario.list) {
         ...
         for (file in var.files) { 
            ...
            work <- paste0("qsub -N ",job.name," -v gcm='",gcm,"',run='",run,"',scenario='",scenario,"',type='",type,"',climname='",clim.name,"' run.climdex.variables.pbs")
            print(work)
            system(work)
            Sys.sleep(1)
            browser()
