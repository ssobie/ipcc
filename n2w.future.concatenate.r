...
gcm.list <- c('GISS-E2-H','GISS-E2-R')
...
if (1==1) {
  ...
  ##read.dir <- paste0('/storage/data/climate/downscale/BCCAQ2+PRISM/CMIP5/global/annual/',gcm,'/')
  ##write.dir <- paste0('/storage/data/climate/downscale/BCCAQ2+PRISM/CMIP5/global/annual/',gcm,'/')

  full.files <- list.files(path=read.dir,pattern=paste(varname,'_OImon_',gcm,sep=''))
  split.apart <- unlist(strsplit(unlist(full.files),'_'))
  runs <- unique(split.apart[grep('r*i1p*',split.apart)])

  move.to <- paste("rsync -av ",read.dir,'/',varname,'_OImon_',gcm,'*nc ',tmp.dir,sep='')
  print(move.to)
  system(move.to)

  ##PR files
  all.files <- as.list(list.files(path=tmp.dir,pattern=paste(varname,'_OImon',sep='')))
