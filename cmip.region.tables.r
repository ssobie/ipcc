...
degree_day_regional_values <- function(var.name,area.info,
  ...
  proj.dd.file <- dd.files[grep(proj.int,dd.files)]
...
return_period_regional_values <- function(var.name,area.info,
  ...
  proj.rp.file <- rp.files[grep(proj.int,rp.files)]
...
climdex_regional_values <- function(var.name,area.info,
  ...
  proj.cx.file <- cx.files[grep(proj.int,cx.files)]
...
if (testing) {
   ...
   regions <- c("canada_boundary","bc","prairies","ontario","quebec","atlantic_canada",
                "northern_canada") ##

   tmpdir <- "/local_temp/ssobie/cmip_regions"
   var.class <- 'return_periods'
   ##------------------------------------
} else {
...
tmp.dir <- paste0(tmpdir,'/tables_',cmip,'_',scenario,'_',region,'/')
...
write.base <- paste0("/storage/data/projects/rci/data/cas/",tolower(cmip),"/tables/",region,"/")
