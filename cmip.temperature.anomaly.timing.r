...
##cmip5.list <- list(c('NorESM1-ME','r1i1p1'))
...
cmip6.list <- list(c('CanESM5','r1i1p2f1'),
                   c('CanESM5','r2i1p2f1'),
                   c('CanESM5','r3i1p2f1'),
                   c('CanESM5','r4i1p2f1'),
                   c('CanESM5','r5i1p2f1'),
...
anomaly.years <- matrix(0,nrow=length(gcm.list),ncol=length(anoms)+2)
anomaly.values <- matrix(0,nrow=length(gcm.list),ncol=length(anoms)+2)
...
for (j in seq_along(gcm.list)) {
   ...
   if (grepl('(FGOALS-g2|GFDL-ESM2G|GFDL-ESM2M|FGOALS-g3)',gcm)) { 
   ...
   anomaly.years[j,] <- c(gcm,run,tas.timing$years)
   anomaly.values[j,] <- c(gcm,run,tas.timing$values)
...
browser()
rv <- rbind(c('Model','Run','1_Degree','2_Degrees','3_Degrees','4_Degrees'),anomaly.years)
write.file <- paste0(write.dir,"global_",experiment,"_",scenario,"_tas_degree_anomaly_years.csv")
write.table(rv,file=write.file,sep=',',quote=F,row.name=F,col.name=F)

rv <- rbind(c('Model','Run','1_Degree','2_Degrees','3_Degrees','4_Degrees'),anomaly.values)
write.file <- paste0(write.dir,"global_",experiment,"_",scenario,"_tas_degree_anomaly_values.csv")
write.table(rv,file=write.file,sep=',',quote=F,row.name=F,col.name=F)
