...
base.dir <- '/storage/data/climate/CMIP6/assembled/'
...
for (v in seq_along(var.list)) {
   ...
   for (g in seq_along(gcms)) {
     ...
     ##GCM files
     gcm.files <- list.files(path=paste0(base.dir,gcm),pattern=paste0(variable,'_day'))
     if (length(gcm.files) !=0) {
     ...
     for (i in 1:length(gcm.files)) {
        ...
        scenario <- gsub("\\+","_",file.split[grepl('historical',file.split)])

        future <- grepl('ssp',scenario)
  ...
  write.file <- paste0(write.dir,variable,'_assembled_daily_CMIP6_model_inventory_',format(Sys.time(),'%Y-%m-%d'),'.csv')
