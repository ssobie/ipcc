...
for (g in seq_along(gcm.list)) {
  gcm <- gcm.list[g]
  print(gcm)
  gcm.dir <- paste0('/storage/data/climate/downscale/BCCAQ2/CMIP5/global/historicalNAT/annual/',gcm,'/')
  
  past.file <- list.files(path=gcm.dir,pattern=paste0('tas_ann_',obs.grid,'_grid_',gcm,'_historicalNat*'),full.name=T)
  print(past.file)
  ...
  my.series <- list(time=1:length(ag.past$global),data=ag.past$global)
  past.fit <- lm(arctic~global,data=my.past)
  
  past.zyp <- zyp.sen(data~time,data=my.series)
...
write.file <- paste0('/storage/data/projects/rci/data/nrcan/nasa_gistemp/slopes/',obs.grid,'_slopes_historicalNAT_2005.csv')
...
plot.file <- '/storage/data/projects/rci/data/nrcan/nasa_gistemp/hadcru4_historicalNAT_arctic_global_anomaly.png'
  png(plot.file,width=800,height=800)
  par(mar=c(5,5,4,2))
  plot(had.global.mean,had.arctic.mean,xlim=c(-1,1),ylim=c(-2,2),cex=2,main='NASA HadCRU4 and HistoricalNat GCM Anomalies',
