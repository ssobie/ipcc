...
##***
##Climatologies are calculated from the intervals specific to each model.
##Intervals are determined by the timing of one, two, and three degrees
##of annual average temperature warming.
...
##cmip5.list <- list(list(gcm='NorESM1-ME',runs='r1i1p1'))
...
if (cmip=='CMIP6') {
   ...
   deg.file <- paste0("/storage/data/projects/rci/data/cas/cmip6/degree_climatologies/",
                      "global_cmip6_",scenario,"_tas_degree_anomaly_years.csv")
} 

if (cmip=='CMIP5') {
   base.dir <- '/storage/data/climate/CMIP5/daily/Derived/'
   gcm.list <- cmip5.list
   deg.file <- paste0("/storage/data/projects/rci/data/cas/cmip5/degree_climatologies/",
                      "global_cmip5_",scenario,"_tas_degree_anomaly_years.csv")
} 
...
for (g in seq_along(gcm.list)) {
   ...
   for (j in seq_along(runs)) {
      ...
      ##-------------------
      ##Read in degree anomaly intervals
      ...
      write.dir <- paste0(read.dir,'degree_anomaly_climatologies/')
      ...
      for (i in seq_along(files)) {
         ...
         if (type=='climdex') {
            ...
            for (interval in intervals) { 
               deg <- c('one','two','three')[which(intervals %in% interval)]

               time.file <- subset_by_time(var.file,interval,var.freq,
                                           tmp.dir,tmp.dir)
               anom.file <- gsub(pattern=run,replacement=paste0(run,'_',deg,'_deg'),time.file)
               if (var.freq=='mon') {
                  ##Monthly climatologies
                  
                  mon.file <- gsub(pattern=paste0(var.name,'_mon'),
                                   replacement=paste(var.name,'_monthly_average_climatology',sep=''),anom.file)
                  ...
                  seas.file <- gsub(pattern=paste0(var.name,'_mon'),
                                   replacement=paste(var.name,'_seasonal_average_climatology',sep=''),anom.file)
                  ...
                  ann.file <- gsub(pattern=paste0(var.name,'_mon'),
                                   replacement=paste(var.name,'_annual_average_climatology',sep=''),anom.file)
                  ...
                  clim.file <- gsub(pattern=paste0('_',var.freq,'_'),replacement='_annual_average_climatology_',anom.file)
         ...
         if (type=='annual') {
            ...
            for (interval in intervals) {
               ...
               anom.file <- gsub(pattern=run,replacement=paste0(run,'_',deg,'_deg'),time.file)
               clim.file <- gsub(pattern=paste0(var.freq,'_',var.avg),replacement=paste0(var.freq,'_',var.avg,'_climatology'),anom.file)
         ...
         if (type=='degree_days') {
            ...
            for (interval in intervals) {
               ...
               anom.file <- gsub(pattern=run,replacement=paste0(run,'_',deg,'_deg'),time.file)
               clim.file <- gsub(pattern=paste0(var.freq,'_'),replacement=paste0(var.freq,'_climatology_'),anom.file)
               make_climatology(time.file,coord.file,'timmean',tmp.dir,tmp.dir)
               fix_gcm_coords(coord.file,clim.file,tmp.dir)
